...
class DiscoveryFilter(BaseModel):
    ...
    min_reputation: Optional[float] = Field(None, ge=0.0, le=10.0, description="Minimum reputation score")
    max_reputation: Optional[float] = Field(None, ge=0.0, le=10.0, description="Maximum reputation score")
    ...
    availability: Optional[bool] = Field(None, description="Filter by availability status")
    verified_only: bool = Field(False, description="Only return verified agents")
...
class ExternalAgentResponse(BaseModel):
    ...
    external_id: str = Field(..., description="Agent ID on the external platform")
...
class ExternalAgentDiscoveryRequest(BaseModel):
    ...
    agent_types: Optional[List[ExternalAgentType]] = Field(
        ...
        description="Specific agent types to look for"
    )
    ...
    include_unavailable: bool = Field(False, description="Include unavailable agents")
...
class PlatformConnectorInfo(BaseModel):
    ...
    supported_agent_types: List[ExternalAgentType] = Field(default_factory=list)
...
class AgentInvitationRequest(BaseModel):
    ...
    platform: str = Field(..., description="Platform hosting the agent")
    invitation_type: str = Field("negotiation", description="Type of invitation")
...
class ExternalAgentRegistration(BaseModel):
    ...
    platform_agent_id: str = Field(..., description="Agent ID on external platform")
    ...
    platform_verification_token: str = Field(..., description="Platform-provided verification token")
