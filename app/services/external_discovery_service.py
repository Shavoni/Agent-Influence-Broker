...
class ExternalDiscoveryService:
        ...
        try:
            ...
            for i, result in enumerate(platform_results):
                if isinstance(result, Exception):
                    logger.error(
                        f"Discovery failed for platform {platforms[i]}: {result}"
                    )
                    continue
            ...
            logger.info(
                f"Discovered {len(sorted_agents)} external agents across {len(platforms)} platforms"
            )
    ...
    def _apply_filters(
        ...
        if filters.min_reputation is not None:
            filtered = [
                a
                for a in filtered
                if a.reputation_score >= filters.min_reputation
            ]
        ...
        if filters.max_reputation is not None:
            filtered = [
                a
                for a in filtered
                if a.reputation_score <= filters.max_reputation
            ]
        ...
        if filters.keywords:
            ...
            filtered = [
                ...
                for a in filtered
                if any(
                    kw in a.name.lower() or kw in a.description.lower()
        ...
        if filters.availability is not None:
            filtered = [
                a for a in filtered if a.is_available == filters.availability
            ]
    ...
    def _generate_cache_key(
        ...
        key_parts = [
            ...
            "|".join(sorted([t.value for t in agent_types]))
            if agent_types
            else "",
        ...
        for platform_name, connector in self.connectors.items():
            try:
                ...
                status = (
                    ...
                    if is_available
                    else ConnectorStatus.UNAVAILABLE
                )
                ...
                logger.error(
                    f"Failed to get status for platform {platform_name}: {e}"
                )
    ...
    async def connect_to_platform(
        self, platform_name: str, config: Dict[str, Any], user_id: str
    ) -> Dict[str, Any]:
    ...
    async def get_agent_details(
        self, agent_id: str, platform: str, user_id: str
    ...
    async def invite_agent(
        ...
        invitation_result = await connector.invite_agent(
            agent_id=agent_id, inviter_id=inviter_id, message=message
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        platform_statuses = [
            p["status"] for p in health_status["platforms"].values()
        ]
