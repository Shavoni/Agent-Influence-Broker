...
async def discover_external_agents(
    ...
    discovery_service: ExternalDiscoveryService = Depends(
    ...
    try:
        logger.info(
            f"User {current_user.id} discovering external agents: {discovery_request.platforms}"
        )
        ...
        logger.error(
            f"External agent discovery failed for user {current_user.id}: {e}"
        )
        raise HTTPException(
...
@router.get(
    "/platforms/{platform_name}/agents",
    response_model=List[ExternalAgentResponse],
)
async def get_platform_agents(
    ...
    limit: int = Query(
        50, ge=1, le=200, description="Maximum number of agents to return"
    ),
    ...
    try:
        ...
        logger.error(
            f"Failed to get agents from platform {platform_name}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def connect_to_platform(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.id} connected to platform {platform_name}"
        )
...
async def disconnect_from_platform(
    ...
    try:
        await discovery_service.disconnect_from_platform(
            platform_name=platform_name, user_id=current_user.id
        )

        logger.info(
            f"User {current_user.id} disconnected from platform {platform_name}"
        )
...
async def get_external_agent_details(
    ...
    platform: str = Query(
        ..., description="Platform where the agent is hosted"
    ),
    discovery_service: ExternalDiscoveryService = Depends(
        get_discovery_service
    ),
...
async def invite_external_agent(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.id} invited external agent {external_agent_id}"
        )
