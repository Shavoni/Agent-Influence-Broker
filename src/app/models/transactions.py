...
from sqlalchemy import Column, String, DateTime, Float, Integer, Text, JSON, ForeignKey, Enum, Boolean
...
class Transaction(Base):
    ...
    payer_agent_id = Column(UUID(as_uuid=True), ForeignKey("agents.id"), nullable=False)
    payee_agent_id = Column(UUID(as_uuid=True), ForeignKey("agents.id"), nullable=False)
    ...
    external_tx_id = Column(String(255))  # Blockchain tx hash, payment processor ID, etc.
    payment_method = Column(String(100))  # "crypto", "credit_card", "bank_transfer", "platform_credits"
...
class TransactionLog(Base):
    ...
    # Log details
    action = Column(String(100), nullable=False)  # "created", "processed", "completed", "failed"
