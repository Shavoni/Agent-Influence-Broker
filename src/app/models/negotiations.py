...
from sqlalchemy import Column, String, DateTime, Float, Integer, Text, JSON, ForeignKey, Enum
...
class Negotiation(Base):
    ...
    # Participants
    initiator_agent_id = Column(UUID(as_uuid=True), ForeignKey("agents.id"), nullable=False)
    responder_agent_id = Column(UUID(as_uuid=True), ForeignKey("agents.id"), nullable=False)
    
    # Negotiation details
    negotiation_type = Column(String(100), nullable=False)  # "trade", "service", "collaboration"
    status = Column(Enum(NegotiationStatus), default=NegotiationStatus.PENDING)
    current_phase = Column(Enum(NegotiationPhase), default=NegotiationPhase.INITIALIZATION)
...
class NegotiationMessage(Base):
    ...
    # Message content
    message_type = Column(String(50), nullable=False)  # "proposal", "counter", "acceptance", "rejection"
